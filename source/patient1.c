...
#define LOGIN_FILE_PATH	"./input/patient1.txt"
#define INSUR_FILE_PATH	"./input/patient1insurance.txt"	
#define BUF_SIZE		128
...
//		perror("CommWithDoc socket()");
	...
	//read insurance plan of patient1 from file
...
//		perror(INSUR_FILE_PATH);
	...
	//send insurance plan of patient1 to doctor
	...
	printf("Phase 3: The cost estimation request from Patient 1 with insurance plan %d has been sent to the doctor with port number %d and IP address %s.\n", plan, ntohs(server_saddr.sin_port), inet_ntoa(server_saddr.sin_addr));
...
int ClientLoginAuth(int client_sock_d)
{
	...
	//read login info of patient1 from file
...
int ClientMakeAppointment(int client_sock_d, char* doc_name, int* doc_port)
{
	...
	printf("Phase 2: The requested appointment is available and reserverd to Patient 1. The assigned doctor port number is %d\n", *doc_port);
